#!/usr/bin/env python3
# D153_SANDBOX_CANDIDATE_FINAL_REAL_APPLY_AUDIT_SCOPE_BOOT.py
# Final real-apply audit scope. No apply, no route insert, no core mutation by AI,
# no shell/network/secrets/git action by AI. Opens D154 archive scope only.
...
print("D153 SANDBOX CANDIDATE FINAL REAL APPLY AUDIT SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_final_real_apply_audit_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d153_sandbox_candidate_final_real_apply_audit_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_final_real_apply_audit_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d153_sandbox_candidate_final_real_apply_audit_scope", "-v"], check=True)

print("\n== run D153 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_final_real_apply_audit_scope import create_sandbox_candidate_final_real_apply_audit_scope\n"
    "r=create_sandbox_candidate_final_real_apply_audit_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d153_sandbox_candidate_final_real_apply_audit_scope.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/sandbox_candidate_final_real_apply_audit_scope.py",
    "tests/test_d153_sandbox_candidate_final_real_apply_audit_scope.py",
    "reports/d153_sandbox_candidate_final_real_apply_audit_scope.json",
    "reports/d153_sandbox_candidate_real_apply_audit_ledger.json",
    "reports/d153_sandbox_candidate_real_apply_replay_index.json",
    "reports/d153_d154_sandbox_candidate_real_apply_chain_archive_scope.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D153_SANDBOX_CANDIDATE_FINAL_REAL_APPLY_AUDIT_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D153 sandbox candidate final real apply audit scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D153 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD153 SANDBOX CANDIDATE FINAL REAL APPLY AUDIT SCOPE BOOT DONE")

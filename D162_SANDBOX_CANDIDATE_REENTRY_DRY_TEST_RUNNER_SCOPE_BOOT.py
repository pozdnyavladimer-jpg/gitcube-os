#!/usr/bin/env python3
# D162_SANDBOX_CANDIDATE_REENTRY_DRY_TEST_RUNNER_SCOPE_BOOT.py
#
# D162 consumes D161 sandbox-candidate reentry test-plan artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_dry_test_runner_scope.py
# - tests/test_d162_sandbox_candidate_reentry_dry_test_runner_scope.py
# - reports/d162_sandbox_candidate_reentry_dry_test_runner_scope.json
# - reports/d162_sandbox_candidate_reentry_dry_test_report.json
# - reports/d162_sandbox_candidate_reentry_test_safety_receipt.json
# - reports/d162_d163_provider_response_reentry_scope.json
#
# D162 is a dry test runner scope gate.
# It may evaluate the D161 test matrix metadata, but it does NOT execute candidate code,
# does NOT write candidate payload files, does NOT call a provider, and does NOT apply.
#
# No network, no secret read, no shell, no route insert,
# no protected core mutation by AI, no git action by AI.
#
# Opens D163_PROVIDER_RESPONSE_REENTRY_SCOPE only.
...
print("D162 SANDBOX CANDIDATE REENTRY DRY TEST RUNNER SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_dry_test_runner_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d162_sandbox_candidate_reentry_dry_test_runner_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_dry_test_runner_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d162_sandbox_candidate_reentry_dry_test_runner_scope", "-v"], check=True)

print("\n== run D162 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_dry_test_runner_scope import create_sandbox_candidate_reentry_dry_test_runner_scope\n"
    "r=create_sandbox_candidate_reentry_dry_test_runner_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d162_sandbox_candidate_reentry_dry_test_runner_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D162_SANDBOX_CANDIDATE_REENTRY_DRY_TEST_RUNNER_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D162 sandbox candidate reentry dry test runner scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D162 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD162 SANDBOX CANDIDATE REENTRY DRY TEST RUNNER SCOPE BOOT DONE")

#!/usr/bin/env python3
# D173_SANDBOX_CANDIDATE_REENTRY_POST_APPLY_VERIFICATION_SCOPE_BOOT.py
#
# D173 consumes D172 guarded-apply artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_post_apply_verification_scope.py
# - tests/test_d173_sandbox_candidate_reentry_post_apply_verification_scope.py
# - reports/d173_sandbox_candidate_reentry_post_apply_verification_scope.json
# - reports/d173_sandbox_candidate_reentry_post_apply_verification_report.json
# - reports/d173_sandbox_candidate_reentry_apply_integrity_receipt.json
# - reports/d173_d174_sandbox_candidate_reentry_final_apply_audit_scope.json
#
# D173 verifies the guarded apply record.
# In this reentry chain D172 recorded guarded apply intent/receipt only:
# candidate_apply_recorded=True, candidate_apply_executed=False.
#
...
#
# Opens D174_SANDBOX_CANDIDATE_REENTRY_FINAL_APPLY_AUDIT_SCOPE only.
...
print("D173 SANDBOX CANDIDATE REENTRY POST APPLY VERIFICATION SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_post_apply_verification_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d173_sandbox_candidate_reentry_post_apply_verification_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_post_apply_verification_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d173_sandbox_candidate_reentry_post_apply_verification_scope", "-v"], check=True)

print("\n== run D173 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_post_apply_verification_scope import create_sandbox_candidate_reentry_post_apply_verification_scope\n"
    "r=create_sandbox_candidate_reentry_post_apply_verification_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d173_sandbox_candidate_reentry_post_apply_verification_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D173_SANDBOX_CANDIDATE_REENTRY_POST_APPLY_VERIFICATION_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D173 sandbox candidate reentry post apply verification scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D173 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD173 SANDBOX CANDIDATE REENTRY POST APPLY VERIFICATION SCOPE BOOT DONE")

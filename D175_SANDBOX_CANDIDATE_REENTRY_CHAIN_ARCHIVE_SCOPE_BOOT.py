#!/usr/bin/env python3
# D175_SANDBOX_CANDIDATE_REENTRY_CHAIN_ARCHIVE_SCOPE_BOOT.py
#
# D175 consumes D174 final-apply-audit artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_chain_archive_scope.py
# - tests/test_d175_sandbox_candidate_reentry_chain_archive_scope.py
# - reports/d175_sandbox_candidate_reentry_chain_archive_scope.json
# - reports/d175_sandbox_candidate_reentry_chain_archive_manifest.json
# - reports/d175_sandbox_candidate_reentry_chain_closure_receipt.json
# - reports/d175_d176_sandbox_candidate_reentry_controlled_next_cycle_scope.json
#
# D175 archives the reentry chain state as a record-only closure.
...
#
# Opens D176_SANDBOX_CANDIDATE_REENTRY_CONTROLLED_NEXT_CYCLE_SCOPE only.
...
print("D175 SANDBOX CANDIDATE REENTRY CHAIN ARCHIVE SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_chain_archive_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d175_sandbox_candidate_reentry_chain_archive_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_chain_archive_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d175_sandbox_candidate_reentry_chain_archive_scope", "-v"], check=True)

print("\n== run D175 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_chain_archive_scope import create_sandbox_candidate_reentry_chain_archive_scope\n"
    "r=create_sandbox_candidate_reentry_chain_archive_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d175_sandbox_candidate_reentry_chain_archive_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D175_SANDBOX_CANDIDATE_REENTRY_CHAIN_ARCHIVE_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D175 sandbox candidate reentry chain archive scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D175 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD175 SANDBOX CANDIDATE REENTRY CHAIN ARCHIVE SCOPE BOOT DONE")

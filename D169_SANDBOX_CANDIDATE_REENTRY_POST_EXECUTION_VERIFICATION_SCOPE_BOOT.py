#!/usr/bin/env python3
# D169_SANDBOX_CANDIDATE_REENTRY_POST_EXECUTION_VERIFICATION_SCOPE_BOOT.py
#
# D169 consumes D168 controlled sandbox execution artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_post_execution_verification_scope.py
# - tests/test_d169_sandbox_candidate_reentry_post_execution_verification_scope.py
# - reports/d169_sandbox_candidate_reentry_post_execution_verification_scope.json
# - reports/d169_sandbox_candidate_reentry_post_execution_verification_report.json
# - reports/d169_sandbox_candidate_reentry_execution_integrity_receipt.json
# - reports/d169_d170_sandbox_candidate_reentry_apply_preflight_scope.json
#
# D169 verifies that D168 sandbox execution stayed no-op / no-apply / no-core-mutation.
...
#
# Opens D170_SANDBOX_CANDIDATE_REENTRY_APPLY_PREFLIGHT_SCOPE only.
...
print("D169 SANDBOX CANDIDATE REENTRY POST EXECUTION VERIFICATION SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_post_execution_verification_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d169_sandbox_candidate_reentry_post_execution_verification_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_post_execution_verification_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d169_sandbox_candidate_reentry_post_execution_verification_scope", "-v"], check=True)

print("\n== run D169 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_post_execution_verification_scope import create_sandbox_candidate_reentry_post_execution_verification_scope\n"
    "r=create_sandbox_candidate_reentry_post_execution_verification_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d169_sandbox_candidate_reentry_post_execution_verification_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D169_SANDBOX_CANDIDATE_REENTRY_POST_EXECUTION_VERIFICATION_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D169 sandbox candidate reentry post execution verification scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D169 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD169 SANDBOX CANDIDATE REENTRY POST EXECUTION VERIFICATION SCOPE BOOT DONE")

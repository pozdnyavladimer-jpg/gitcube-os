#!/usr/bin/env python3
# D126_PROPOSAL_TO_SANDBOX_CANDIDATE_SCOPE_BOOT.py
#
# Creates D126 Proposal To Sandbox Candidate Scope.
#
# D126 consumes D125 provider-response intake artifacts and creates
# sandbox-candidate planning/static-scan artifacts:
# - runtime_experimental/proposal_to_sandbox_candidate_scope.py
# - tests/test_d126_proposal_to_sandbox_candidate_scope.py
# - reports/d126_proposal_to_sandbox_candidate_scope.json
# - reports/d126_sandbox_candidate_write_plan.json
# - reports/d126_sandbox_candidate_static_scan.json
# - reports/d126_d127_sandbox_candidate_human_review_scope.json
#
# This is PROPOSAL TO SANDBOX CANDIDATE SCOPE ONLY.
...
#
# D126 opens D127 Sandbox Candidate Human Review Scope.
...
print("D126 PROPOSAL TO SANDBOX CANDIDATE SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/proposal_to_sandbox_candidate_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d126_proposal_to_sandbox_candidate_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/proposal_to_sandbox_candidate_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d126_proposal_to_sandbox_candidate_scope", "-v"], check=True)

print("\n== run D126 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.proposal_to_sandbox_candidate_scope import create_proposal_to_sandbox_candidate_scope\n"
    "r=create_proposal_to_sandbox_candidate_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d126_proposal_to_sandbox_candidate_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D126_PROPOSAL_TO_SANDBOX_CANDIDATE_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D126 proposal to sandbox candidate scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D126 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD126 PROPOSAL TO SANDBOX CANDIDATE SCOPE BOOT DONE")

#!/usr/bin/env python3
# D95_HUMAN_SIGNED_EXECUTION_INTENT_BOOT.py
#
# Creates D95 Human Signed Execution Intent.
#
# D95 converts D94 review request into a signed human-intent artifact.
...
print("D95 HUMAN SIGNED EXECUTION INTENT BOOT: repo =", ROOT)

Path("runtime_experimental/human_signed_execution_intent.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d95_human_signed_execution_intent.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/human_signed_execution_intent.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d95_human_signed_execution_intent", "-v"], check=True)

print("\n== run D95 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.human_signed_execution_intent import create_human_signed_execution_intent\n"
    "r=create_human_signed_execution_intent()\n"
...
print("\n== report preview ==")
p = Path("reports/d95_human_signed_execution_intent.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/human_signed_execution_intent.py",
    "tests/test_d95_human_signed_execution_intent.py",
    "reports/d95_human_signed_execution_intent.json",
    "reports/d95_execution_intent_signature.json",
    "reports/d95_apply_still_blocked.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D95_HUMAN_SIGNED_EXECUTION_INTENT_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D95 human signed execution intent"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D95 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD95 HUMAN SIGNED EXECUTION INTENT BOOT DONE")

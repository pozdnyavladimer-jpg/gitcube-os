#!/usr/bin/env python3
# D94_FINAL_EXECUTION_APPROVAL_REQUEST_BOOT.py
#
# Creates D94 Final Execution Approval Request.
#
# D94 asks for explicit human review of the final execution gate.
...
print("D94 FINAL EXECUTION APPROVAL REQUEST BOOT: repo =", ROOT)

Path("runtime_experimental/final_execution_approval_request.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d94_final_execution_approval_request.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/final_execution_approval_request.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d94_final_execution_approval_request", "-v"], check=True)

print("\n== run D94 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.final_execution_approval_request import create_final_execution_approval_request\n"
    "r=create_final_execution_approval_request()\n"
...
print("\n== report preview ==")
p = Path("reports/d94_final_execution_approval_request.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/final_execution_approval_request.py",
    "tests/test_d94_final_execution_approval_request.py",
    "reports/d94_final_execution_approval_request.json",
    "reports/d94_explicit_human_execution_phrase.json",
    "reports/d94_final_apply_blockers_report.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D94_FINAL_EXECUTION_APPROVAL_REQUEST_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D94 final execution approval request"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D94 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD94 FINAL EXECUTION APPROVAL REQUEST BOOT DONE")

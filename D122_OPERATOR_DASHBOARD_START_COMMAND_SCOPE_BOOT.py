#!/usr/bin/env python3
# D122_OPERATOR_DASHBOARD_START_COMMAND_SCOPE_BOOT.py
#
# Creates D122 Operator Dashboard / Start Command Scope.
#
# D122 consumes D121 AI propose-only provider adapter artifacts and creates
# a one-entry dashboard/start-command scope:
# - runtime_experimental/operator_dashboard_start_command_scope.py
# - tests/test_d122_operator_dashboard_start_command_scope.py
# - reports/d122_operator_dashboard_start_command_scope.json
# - reports/d122_single_entry_prompt_to_proposal_flow.json
# - reports/d122_dashboard_preflight_checklist.json
# - reports/d122_d123_provider_config_manual_enable_scope.json
#
# This is OPERATOR DASHBOARD START COMMAND SCOPE ONLY.
...
#
# D122 opens D123 Provider Config Manual Enable Scope.
...
print("D122 OPERATOR DASHBOARD START COMMAND SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/operator_dashboard_start_command_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d122_operator_dashboard_start_command_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/operator_dashboard_start_command_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d122_operator_dashboard_start_command_scope", "-v"], check=True)

print("\n== run D122 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.operator_dashboard_start_command_scope import create_operator_dashboard_start_command_scope\n"
    "r=create_operator_dashboard_start_command_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d122_operator_dashboard_start_command_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D122_OPERATOR_DASHBOARD_START_COMMAND_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D122 operator dashboard start command scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D122 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD122 OPERATOR DASHBOARD START COMMAND SCOPE BOOT DONE")

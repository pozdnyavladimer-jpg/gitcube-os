#!/usr/bin/env python3
# D170_SANDBOX_CANDIDATE_REENTRY_APPLY_PREFLIGHT_SCOPE_BOOT.py
#
# D170 consumes D169 post-execution verification artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_apply_preflight_scope.py
# - tests/test_d170_sandbox_candidate_reentry_apply_preflight_scope.py
# - reports/d170_sandbox_candidate_reentry_apply_preflight_scope.json
# - reports/d170_sandbox_candidate_reentry_apply_preflight_report.json
# - reports/d170_sandbox_candidate_reentry_apply_blockers.json
# - reports/d170_d171_sandbox_candidate_reentry_human_apply_intent_scope.json
#
# D170 is apply-preflight only. It does NOT apply.
# It prepares the next human-apply-intent gate.
#
...
#
# Opens D171_SANDBOX_CANDIDATE_REENTRY_HUMAN_APPLY_INTENT_SCOPE only.
...
print("D170 SANDBOX CANDIDATE REENTRY APPLY PREFLIGHT SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_apply_preflight_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d170_sandbox_candidate_reentry_apply_preflight_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_apply_preflight_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d170_sandbox_candidate_reentry_apply_preflight_scope", "-v"], check=True)

print("\n== run D170 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_apply_preflight_scope import create_sandbox_candidate_reentry_apply_preflight_scope\n"
    "r=create_sandbox_candidate_reentry_apply_preflight_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d170_sandbox_candidate_reentry_apply_preflight_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D170_SANDBOX_CANDIDATE_REENTRY_APPLY_PREFLIGHT_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D170 sandbox candidate reentry apply preflight scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D170 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD170 SANDBOX CANDIDATE REENTRY APPLY PREFLIGHT SCOPE BOOT DONE")

...
def contains_forbidden(obj, forbidden_fields, forbidden_actions):
    ...
    guardrail_keys = set(str(x).lower() for x in GUARDRAIL_FALSE_KEYS)
    ...
    def walk(x, path=""):
        if isinstance(x, dict):
            for k, v in x.items():
                ...
                # These guardrail keys are required to exist and be False.
                # Their presence describes blocked capability, not an action request.
                is_required_guardrail_key = path == "guardrails." and k_lower in guardrail_keys

                if k_lower in tokens and not is_required_guardrail_key:

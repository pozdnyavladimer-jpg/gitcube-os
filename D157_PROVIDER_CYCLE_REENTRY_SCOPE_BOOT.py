#!/usr/bin/env python3
# D157_PROVIDER_CYCLE_REENTRY_SCOPE_BOOT.py
# Provider cycle reentry scope only:
...
# Opens D158 proposal cycle reentry intake scope only.
...
def validate_d156(d156, intake_manifest, safety_reset, d157_scope):
    ...
    if not safety_reset:
        ...
        for k in FALSE_KEYS + STATUS_FALSE_D156:
            if safety_reset.get(k) is not False:
...
print("D157 PROVIDER CYCLE REENTRY SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/provider_cycle_reentry_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d157_provider_cycle_reentry_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/provider_cycle_reentry_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d157_provider_cycle_reentry_scope", "-v"], check=True)

print("\n== run D157 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.provider_cycle_reentry_scope import create_provider_cycle_reentry_scope\n"
    "r=create_provider_cycle_reentry_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d157_provider_cycle_reentry_scope.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/provider_cycle_reentry_scope.py",
    "tests/test_d157_provider_cycle_reentry_scope.py",
    "reports/d157_provider_cycle_reentry_scope.json",
    "reports/d157_provider_reentry_config_manifest.json",
    "reports/d157_provider_reentry_dry_ping_scope.json",
    "reports/d157_d158_proposal_cycle_reentry_intake_scope.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D157_PROVIDER_CYCLE_REENTRY_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D157 provider cycle reentry scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D157 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD157 PROVIDER CYCLE REENTRY SCOPE BOOT DONE")

#!/usr/bin/env python3
# D154_SANDBOX_CANDIDATE_REAL_APPLY_CHAIN_ARCHIVE_SCOPE_BOOT.py
# Real apply chain archive scope only: no second apply, no route insert,
# no protected core mutation by AI, no shell/network/secret/git action by AI.
...
print("D154 SANDBOX CANDIDATE REAL APPLY CHAIN ARCHIVE SCOPE BOOT: repo =", ROOT)
Path("runtime_experimental/sandbox_candidate_real_apply_chain_archive_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d154_sandbox_candidate_real_apply_chain_archive_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_real_apply_chain_archive_scope.py"], check=True)
print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d154_sandbox_candidate_real_apply_chain_archive_scope", "-v"], check=True)
print("\n== run D154 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_real_apply_chain_archive_scope import create_sandbox_candidate_real_apply_chain_archive_scope\n"
    "r=create_sandbox_candidate_real_apply_chain_archive_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d154_sandbox_candidate_real_apply_chain_archive_scope.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/sandbox_candidate_real_apply_chain_archive_scope.py",
    "tests/test_d154_sandbox_candidate_real_apply_chain_archive_scope.py",
    "reports/d154_sandbox_candidate_real_apply_chain_archive_scope.json",
    "reports/d154_sandbox_candidate_real_apply_chain_archive_manifest.json",
    "reports/d154_sandbox_candidate_real_apply_chain_closure_receipt.json",
    "reports/d154_d155_sandbox_candidate_guarded_autonomy_cycle_closure_scope.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D154_SANDBOX_CANDIDATE_REAL_APPLY_CHAIN_ARCHIVE_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D154 sandbox candidate real apply chain archive scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D154 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD154 SANDBOX CANDIDATE REAL APPLY CHAIN ARCHIVE SCOPE BOOT DONE")

#!/usr/bin/env python3
# D134_SANDBOX_CANDIDATE_WRITE_MATERIALIZATION_SCOPE_BOOT.py
#
# D134 consumes D133 materialization-scope artifacts and performs a guarded
# sandbox-candidate write materialization into the approved candidate root only.
#
# Creates:
# - runtime_experimental/sandbox_candidate_write_materialization_scope.py
# - tests/test_d134_sandbox_candidate_write_materialization_scope.py
# - reports/d134_sandbox_candidate_write_materialization_scope.json
# - reports/d134_sandbox_candidate_write_materialization_receipt.json
# - reports/d134_sandbox_candidate_write_materialization_postcheck.json
# - reports/d134_d135_sandbox_candidate_post_write_validation_scope.json
# - runtime_experimental/ai_sandbox_work/<candidate_id>/candidate_manifest.json
# - runtime_experimental/ai_sandbox_work/<candidate_id>/candidate_summary.md
# - runtime_experimental/ai_sandbox_work/<candidate_id>/candidate_payload.json
#
# WRITE MATERIALIZATION ONLY:
# candidate files may be written once inside runtime_experimental/ai_sandbox_work/<candidate_id>/.
...
#
# D134 opens D135 Sandbox Candidate Post Write Validation Scope.
...
print("D134 SANDBOX CANDIDATE WRITE MATERIALIZATION SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_write_materialization_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d134_sandbox_candidate_write_materialization_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_write_materialization_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d134_sandbox_candidate_write_materialization_scope", "-v"], check=True)

print("\n== run D134 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_write_materialization_scope import create_sandbox_candidate_write_materialization_scope\n"
    "r=create_sandbox_candidate_write_materialization_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d134_sandbox_candidate_write_materialization_scope.json")
if p.exists():
...
try:
    manifest = json.loads(Path("reports/d133_sandbox_candidate_materialization_manifest.json").read_text(encoding="utf-8"))
    for item in manifest.get("planned_materialization_files", []):
...
try:
    ...
    if self_rel.name == "D134_SANDBOX_CANDIDATE_WRITE_MATERIALIZATION_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D134 sandbox candidate write materialization scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D134 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD134 SANDBOX CANDIDATE WRITE MATERIALIZATION SCOPE BOOT DONE")
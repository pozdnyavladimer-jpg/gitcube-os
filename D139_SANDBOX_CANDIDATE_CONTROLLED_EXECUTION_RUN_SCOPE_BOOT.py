#!/usr/bin/env python3
# D139_SANDBOX_CANDIDATE_CONTROLLED_EXECUTION_RUN_SCOPE_BOOT.py
#
# D139 consumes D138 human execution intent artifacts and performs the first
# controlled sandbox candidate execution run.
#
# This is NOT apply. This is NOT route insertion. This does NOT mutate protected core.
# It does NOT run shell, network, provider calls, secrets, git actions by AI,
# rollback, restore, or external commands from the candidate.
#
# Execution meaning in D139:
# - Read the already materialized sandbox candidate files.
# - Interpret the candidate as a declarative sandbox payload.
# - Produce a sandbox execution result inside the candidate sandbox directory.
# - Produce reports and open D140 post-execution verification scope.
...
print("D139 SANDBOX CANDIDATE CONTROLLED EXECUTION RUN SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_controlled_execution_run_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d139_sandbox_candidate_controlled_execution_run_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_controlled_execution_run_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d139_sandbox_candidate_controlled_execution_run_scope", "-v"], check=True)

print("\n== run D139 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_controlled_execution_run_scope import create_sandbox_candidate_controlled_execution_run_scope\n"
    "r=create_sandbox_candidate_controlled_execution_run_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d139_sandbox_candidate_controlled_execution_run_scope.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/sandbox_candidate_controlled_execution_run_scope.py",
    "tests/test_d139_sandbox_candidate_controlled_execution_run_scope.py",
    ...
    "reports/d139_d140_sandbox_candidate_post_execution_verification_scope.json",
]
# Add sandbox execution result file if report identifies it.
try:
    report = json.loads(Path("reports/d139_sandbox_candidate_controlled_execution_run_scope.json").read_text(encoding="utf-8"))
    sandbox_path = report.get("summary", {}).get("sandbox_result_path")
...
try:
    ...
    if self_rel.name == "D139_SANDBOX_CANDIDATE_CONTROLLED_EXECUTION_RUN_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D139 sandbox candidate controlled execution run scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D139 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD139 SANDBOX CANDIDATE CONTROLLED EXECUTION RUN SCOPE BOOT DONE")

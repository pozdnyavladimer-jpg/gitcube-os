#!/usr/bin/env python3
# D127_SANDBOX_CANDIDATE_HUMAN_REVIEW_SCOPE_BOOT.py
#
# Creates D127 Sandbox Candidate Human Review Scope.
#
# D127 consumes D126 proposal-to-sandbox-candidate artifacts and creates
# human-review artifacts:
# - runtime_experimental/sandbox_candidate_human_review_scope.py
# - tests/test_d127_sandbox_candidate_human_review_scope.py
# - reports/d127_sandbox_candidate_human_review_scope.json
# - reports/d127_sandbox_candidate_review_packet.json
# - reports/d127_sandbox_candidate_approval_or_rejection_record.json
# - reports/d127_d128_sandbox_candidate_test_plan_scope.json
#
# This is SANDBOX CANDIDATE HUMAN REVIEW SCOPE ONLY.
# It does NOT write sandbox candidate files.
# It does NOT execute the sandbox candidate.
# It does NOT apply patches.
# It does NOT mutate protected runtime/core/memory/routes.
# It does NOT call provider/network.
# It does NOT read API keys or secrets.
# It does NOT execute shell commands.
# It does NOT commit/push/tag by AI.
#
# D127 opens D128 Sandbox Candidate Test Plan Scope.
...
print("D127 SANDBOX CANDIDATE HUMAN REVIEW SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_human_review_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d127_sandbox_candidate_human_review_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_human_review_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d127_sandbox_candidate_human_review_scope", "-v"], check=True)

print("\n== run D127 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_human_review_scope import create_sandbox_candidate_human_review_scope\n"
    "r=create_sandbox_candidate_human_review_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d127_sandbox_candidate_human_review_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D127_SANDBOX_CANDIDATE_HUMAN_REVIEW_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D127 sandbox candidate human review scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D127 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD127 SANDBOX CANDIDATE HUMAN REVIEW SCOPE BOOT DONE")

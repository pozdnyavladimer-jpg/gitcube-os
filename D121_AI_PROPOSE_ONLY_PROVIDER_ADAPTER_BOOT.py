#!/usr/bin/env python3
# D121_AI_PROPOSE_ONLY_PROVIDER_ADAPTER_BOOT.py
#
# Creates D121 AI Propose-Only Provider Adapter.
#
# D121 consumes the sealed D120 first controlled run artifacts and creates
# a provider-adapter scope that allows ONLY proposal-shaped AI output.
#
# It creates:
# - runtime_experimental/ai_propose_only_provider_adapter.py
# - tests/test_d121_ai_propose_only_provider_adapter.py
# - reports/d121_ai_propose_only_provider_adapter_scope.json
# - reports/d121_provider_adapter_contract.json
# - reports/d121_mock_provider_request_response.json
# - reports/d121_d122_operator_dashboard_start_command_scope.json
#
# This is PROPOSE-ONLY PROVIDER ADAPTER SCOPE.
...
#
# D121 opens D122 Operator Dashboard / Start Command Scope.
...
print("D121 AI PROPOSE-ONLY PROVIDER ADAPTER BOOT: repo =", ROOT)

Path("runtime_experimental/ai_propose_only_provider_adapter.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d121_ai_propose_only_provider_adapter.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/ai_propose_only_provider_adapter.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d121_ai_propose_only_provider_adapter", "-v"], check=True)

print("\n== run D121 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.ai_propose_only_provider_adapter import create_ai_propose_only_provider_adapter_scope\n"
    "r=create_ai_propose_only_provider_adapter_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d121_ai_propose_only_provider_adapter_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D121_AI_PROPOSE_ONLY_PROVIDER_ADAPTER_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D121 AI propose-only provider adapter"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D121 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD121 AI PROPOSE-ONLY PROVIDER ADAPTER BOOT DONE")

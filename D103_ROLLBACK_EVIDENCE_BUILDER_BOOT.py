#!/usr/bin/env python3
# D103_ROLLBACK_EVIDENCE_BUILDER_BOOT.py
# Documentation-only D103 rollback evidence builder.
# No apply, no rollback execution, no restore, no route insert, no protected-core mutation, no network/AI.
...
print("D103 ROLLBACK EVIDENCE BUILDER BOOT: repo =", ROOT)

Path("runtime_experimental/rollback_evidence_builder.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d103_rollback_evidence_builder.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/rollback_evidence_builder.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d103_rollback_evidence_builder", "-v"], check=True)

print("\n== run D103 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.rollback_evidence_builder import create_rollback_evidence_builder\n"
    "r=create_rollback_evidence_builder()\n"
...
print("\n== report preview ==")
p = Path("reports/d103_rollback_evidence_builder.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/rollback_evidence_builder.py",
    "tests/test_d103_rollback_evidence_builder.py",
    "reports/d103_rollback_evidence_builder.json",
    "reports/d103_rollback_evidence_bundle.json",
    "reports/d103_restore_point_reference.json",
    "reports/d103_rollback_readiness_summary.json",
    "reports/d103_d104_final_audit_ledger_scope.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D103_ROLLBACK_EVIDENCE_BUILDER_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D103 rollback evidence builder"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D103 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD103 ROLLBACK EVIDENCE BUILDER BOOT DONE")

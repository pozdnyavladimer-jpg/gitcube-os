#!/usr/bin/env python3
# D97_PROTECTED_CORE_NO_TOUCH_RECONFIRMATION_BOOT.py
#
# Creates D97 Protected Core No-Touch Reconfirmation.
#
# D97 verifies D96 final local regression and creates a protected-core
# hash snapshot + no-route-insert reconfirmation.
#
# It does NOT apply changes, insert routes, mutate protected core,
# call network/AI, execute shell commands, or approve real apply.
...
print("D97 PROTECTED CORE NO-TOUCH RECONFIRMATION BOOT: repo =", ROOT)

Path("runtime_experimental/protected_core_no_touch_reconfirmation.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d97_protected_core_no_touch_reconfirmation.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/protected_core_no_touch_reconfirmation.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d97_protected_core_no_touch_reconfirmation", "-v"], check=True)

print("\n== run D97 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.protected_core_no_touch_reconfirmation import create_protected_core_no_touch_reconfirmation\n"
    "r=create_protected_core_no_touch_reconfirmation()\n"
...
print("\n== report preview ==")
p = Path("reports/d97_protected_core_no_touch_reconfirmation.json")
if p.exists():
...
print("\n== git add/commit ==")
paths = [
    "runtime_experimental/protected_core_no_touch_reconfirmation.py",
    "tests/test_d97_protected_core_no_touch_reconfirmation.py",
    "reports/d97_protected_core_no_touch_reconfirmation.json",
    "reports/d97_protected_file_hash_snapshot.json",
    "reports/d97_no_route_insert_reconfirmation.json",
    "reports/d97_d98_rollback_restore_scope.json",
]
try:
    self_rel = Path(__file__).resolve().relative_to(ROOT)
    if self_rel.name == "D97_PROTECTED_CORE_NO_TOUCH_RECONFIRMATION_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D97 protected core no-touch reconfirmation"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D97 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD97 PROTECTED CORE NO-TOUCH RECONFIRMATION BOOT DONE")

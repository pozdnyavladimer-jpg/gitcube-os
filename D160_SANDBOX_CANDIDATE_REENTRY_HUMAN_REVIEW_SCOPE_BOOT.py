#!/usr/bin/env python3
# D160_SANDBOX_CANDIDATE_REENTRY_HUMAN_REVIEW_SCOPE_BOOT.py
#
# D160 consumes D159 proposal-to-sandbox-candidate reentry artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_human_review_scope.py
# - tests/test_d160_sandbox_candidate_reentry_human_review_scope.py
# - reports/d160_sandbox_candidate_reentry_human_review_scope.json
# - reports/d160_sandbox_candidate_reentry_review_packet.json
# - reports/d160_sandbox_candidate_reentry_no_apply_assertions.json
# - reports/d160_d161_sandbox_candidate_reentry_test_plan_scope.json
#
# D160 is a human-review scope gate only.
# It does NOT write candidate payload files, does NOT execute candidate, does NOT apply.
#
...
#
# Opens D161_SANDBOX_CANDIDATE_REENTRY_TEST_PLAN_SCOPE only.
...
print("D160 SANDBOX CANDIDATE REENTRY HUMAN REVIEW SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_human_review_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d160_sandbox_candidate_reentry_human_review_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_human_review_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d160_sandbox_candidate_reentry_human_review_scope", "-v"], check=True)

print("\n== run D160 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_human_review_scope import create_sandbox_candidate_reentry_human_review_scope\n"
    "r=create_sandbox_candidate_reentry_human_review_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d160_sandbox_candidate_reentry_human_review_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D160_SANDBOX_CANDIDATE_REENTRY_HUMAN_REVIEW_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D160 sandbox candidate reentry human review scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D160 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD160 SANDBOX CANDIDATE REENTRY HUMAN REVIEW SCOPE BOOT DONE")

#!/usr/bin/env python3
# D113_FINAL_APPLY_REVIEW_SCOPE_BOOT.py
#
# Creates D113 Final Apply Review Scope.
#
# D113 consumes D112 dry-run artifacts and creates final-review-only evidence artifacts:
# - runtime_experimental/final_apply_review_scope.py
# - tests/test_d113_final_apply_review_scope.py
# - reports/d113_final_apply_review_scope.json
# - reports/d113_final_apply_evidence_packet.json
# - reports/d113_final_apply_blocker_matrix.json
# - reports/d113_d114_final_human_apply_decision_scope.json
#
# This is FINAL REVIEW ONLY.
...
print("D113 FINAL APPLY REVIEW SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/final_apply_review_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d113_final_apply_review_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/final_apply_review_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d113_final_apply_review_scope", "-v"], check=True)

print("\n== run D113 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.final_apply_review_scope import create_final_apply_review_scope\n"
    "r=create_final_apply_review_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d113_final_apply_review_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D113_FINAL_APPLY_REVIEW_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D113 final apply review scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D113 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD113 FINAL APPLY REVIEW SCOPE BOOT DONE")

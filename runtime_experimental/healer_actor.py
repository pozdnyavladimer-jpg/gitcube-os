import os
import subprocess
from datetime import datetime

SAFE_EXTENSIONS = {".py"}

BLOCKED_FILES = {
    "actor_executor.py",
    "repo_analyzer.py",
    "router.py",
    "run.sh",
    "run_loop.sh",
    "status.py",
    "v_bridge.py",
    "runtime_experimental/pr_actor.py",
    "runtime_experimental/assassin_actor.py",
    "runtime_experimental/healer_actor.py",
    "runtime_experimental/healer_support.py",
    "runtime_experimental/os_sync.py",
    "runtime_experimental/object_store.py",
    "runtime_experimental/github_bridge.py",
}

SAFE_PREFIXES = (
    "examples/",
    "tests/",
)


def run(cmd: str):
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    return result.stdout.strip(), result.stderr.strip(), result.returncode


def normalize_path(path: str) -> str:
    return str(path or "").replace("\\", "/").strip()


def branch_name(task):
    ts = int(datetime.now().timestamp())
    return f"gitcube/healer_{task.get('id')}_{ts}"


def create_branch(name: str):
    return run(f"git checkout -b {name}")


def commit_all(message: str):
    safe_message = str(message).replace('"', "'")
    return run(f'git add . && git commit -m "{safe_message}"')


def push_branch(name: str):
    return run(f"git push origin {name}")


def create_pr(name: str, title: str):
    safe_title = str(title).replace('"', "'")
    return run(
        f'gh pr create --title "{safe_title}" --body "Auto-generated by GitCube HEALER" --base main'
    )


def restore_file(path: str):
    run(f'git checkout -- "{path}"')


def cleanup_branch_to_main():
    run("git checkout main")


def validate_python_file(path: str):
    out, err, code = run(f'python -m py_compile "{path}"')
    if code != 0:
        return False, f"validation_failed:{err or out}"
    return True, "validation_ok"


def is_safe_target(path: str):
    path = normalize_path(path)

    if not path:
        return False, "missing_path"

    if not os.path.exists(path):
        return False, "path_not_found"

    _, ext = os.path.splitext(path)
    if ext not in SAFE_EXTENSIONS:
        return False, f"unsupported_extension:{ext}"

    if path in BLOCKED_FILES:
        return False, "blocked_file"

    if path.startswith("runtime_experimental/"):
        return False, "blocked_runtime_experimental"

    if path.startswith("core/") or path.startswith("runtime/") or path.startswith("app/"):
        return False, "blocked_core_runtime_app"

    if not path.startswith(SAFE_PREFIXES):
        return False, "outside_safe_prefix"

    return True, "safe"


def extract_target_path(task):
    payload = task.get("payload", {}) or {}
    return normalize_path(payload.get("path"))


def should_run_healer(task):
    title = str(task.get("title", "")).lower()
    payload = task.get("payload", {}) or {}
    has_path = bool(str(payload.get("path", "")).strip())

    if not has_path:
        return False

    signals = (
        "pass",
        "bare except",
        "repair",
        "heal",
        "review pass",
        "review bare except",
    )

    return any(s in title for s in signals)


def repair_consistency(path: str):
    changed = False

    with open(path, "r", encoding="utf-8") as f:
        lines = f.readlines()

    new_lines = []

    for line in lines:
        stripped = line.strip()

        if stripped == "except:":
            indent = line[: len(line) - len(line.lstrip())]
            new_lines.append(f"{indent}except Exception as e:\n")
            changed = True
            continue

        if stripped == "pass":
            indent = line[: len(line) - len(line.lstrip())]
            new_lines.append(f"{indent}raise NotImplementedError()\n")
            changed = True
            continue

        new_lines.append(line)

    if changed:
        with open(path, "w", encoding="utf-8") as f:
            f.writelines(new_lines)

    return changed


def run_healer_task(task):
    if not should_run_healer(task):
        return False, "not_supported_task"

    target_path = extract_target_path(task)
    safe_ok, safe_reason = is_safe_target(target_path)
    if not safe_ok:
        return False, safe_reason

    name = branch_name(task)

    out, err, code = create_branch(name)
    if code != 0:
        return False, f"branch_create_failed:{err or out}"

    try:
        changed = repair_consistency(target_path)
    except Exception as e:
        cleanup_branch_to_main()
        return False, f"edit_failed:{e}"

    if not changed:
        cleanup_branch_to_main()
        return False, "no_changes"

    valid, validation_reason = validate_python_file(target_path)
    if not valid:
        restore_file(target_path)
        cleanup_branch_to_main()
        return False, validation_reason

    out, err, code = commit_all(task.get("title", "GitCube HEALER PR"))
    if code != 0:
        cleanup_branch_to_main()
        return False, f"commit_failed:{err or out}"

    out, err, code = push_branch(name)
    if code != 0:
        cleanup_branch_to_main()
        return False, f"push_failed:{err or out}"

    out, err, code = create_pr(name, task.get("title", "GitCube HEALER PR"))
    if code != 0:
        cleanup_branch_to_main()
        return False, f"pr_create_failed:{err or out}"

    cleanup_branch_to_main()
    return True, f"file_changed:{target_path}"

...
def branch_name(task):
    ...
    return f"gitcube/assassin_{task.get('id')}_{ts}"
...
def create_pr(name: str, title: str):
    ...
    return run(
        f'gh pr create --title "{safe_title}" --body "Auto-generated by GitCube ASSASSIN" --base main'
    )
...
def run_assassin_task(task):
    if not should_run_assassin(task):
    ...
    try:
        changed = cleanup_noise(target_path)
    ...
    out, err, code = commit_all(task.get("title", "GitCube ASSASSIN PR"))
    if code != 0:
        cleanup_branch_to_main()
    ...
    out, err, code = create_pr(name, task.get("title", "GitCube ASSASSIN PR"))
    if code != 0:
        cleanup_branch_to_main()

#!/usr/bin/env python3
# D119_POST_APPLY_VERIFICATION_GATE_SCOPE_BOOT.py
#
# Creates D119 Post-Apply Verification Gate Scope.
#
# D119 consumes D118 operator-local evidence artifacts and creates verification-scope-only artifacts:
# - runtime_experimental/post_apply_verification_gate_scope.py
# - tests/test_d119_post_apply_verification_gate_scope.py
# - reports/d119_post_apply_verification_gate_scope.json
# - reports/d119_post_apply_test_results_summary.json
# - reports/d119_post_apply_git_state_summary.json
# - reports/d119_d120_first_controlled_run_seal_scope.json
#
# This is POST-APPLY VERIFICATION GATE SCOPE ONLY.
# It does NOT execute real apply by AI.
# It does NOT execute shell commands by AI.
# It does NOT run rollback/restore by AI.
# It does NOT mutate core/runtime/app/memory/routes.
# It does NOT commit/push by AI.
#
# D119 creates the verification gate container for operator evidence.
# It opens D120 First Controlled Run Seal Scope.
# Real apply by AI remains blocked.
...
print("D119 POST APPLY VERIFICATION GATE SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/post_apply_verification_gate_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d119_post_apply_verification_gate_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/post_apply_verification_gate_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d119_post_apply_verification_gate_scope", "-v"], check=True)

print("\n== run D119 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.post_apply_verification_gate_scope import create_post_apply_verification_gate_scope\n"
    "r=create_post_apply_verification_gate_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d119_post_apply_verification_gate_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D119_POST_APPLY_VERIFICATION_GATE_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D119 post-apply verification gate scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D119 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD119 POST APPLY VERIFICATION GATE SCOPE BOOT DONE")

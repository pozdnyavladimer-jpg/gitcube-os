#!/usr/bin/env python3
# D161_SANDBOX_CANDIDATE_REENTRY_TEST_PLAN_SCOPE_BOOT.py
#
# D161 consumes D160 sandbox-candidate reentry human-review artifacts and creates:
# - runtime_experimental/sandbox_candidate_reentry_test_plan_scope.py
# - tests/test_d161_sandbox_candidate_reentry_test_plan_scope.py
# - reports/d161_sandbox_candidate_reentry_test_plan_scope.json
# - reports/d161_sandbox_candidate_reentry_test_matrix.json
# - reports/d161_sandbox_candidate_reentry_no_touch_assertions.json
# - reports/d161_d162_sandbox_candidate_reentry_dry_test_runner_scope.json
#
# D161 is a test-plan scope gate only.
...
#
# Opens D162_SANDBOX_CANDIDATE_REENTRY_DRY_TEST_RUNNER_SCOPE only.
...
print("D161 SANDBOX CANDIDATE REENTRY TEST PLAN SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_reentry_test_plan_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d161_sandbox_candidate_reentry_test_plan_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_reentry_test_plan_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d161_sandbox_candidate_reentry_test_plan_scope", "-v"], check=True)

print("\n== run D161 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_reentry_test_plan_scope import create_sandbox_candidate_reentry_test_plan_scope\n"
    "r=create_sandbox_candidate_reentry_test_plan_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d161_sandbox_candidate_reentry_test_plan_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D161_SANDBOX_CANDIDATE_REENTRY_TEST_PLAN_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D161 sandbox candidate reentry test plan scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D161 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD161 SANDBOX CANDIDATE REENTRY TEST PLAN SCOPE BOOT DONE")

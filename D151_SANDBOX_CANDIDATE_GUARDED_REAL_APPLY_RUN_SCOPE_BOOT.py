#!/usr/bin/env python3
# D151_SANDBOX_CANDIDATE_GUARDED_REAL_APPLY_RUN_SCOPE_BOOT.py
#
# D151 consumes D150 human-signed real-apply execution artifacts and creates:
# - runtime_experimental/sandbox_candidate_guarded_real_apply_run_scope.py
# - tests/test_d151_sandbox_candidate_guarded_real_apply_run_scope.py
# - reports/d151_sandbox_candidate_guarded_real_apply_run_scope.json
# - reports/d151_sandbox_candidate_real_apply_run_result.json
# - reports/d151_sandbox_candidate_real_apply_run_safety_receipt.json
# - reports/d151_d152_sandbox_candidate_post_real_apply_verification_scope.json
#
# GUARDED REAL APPLY RUN SCOPE:
# This gate records a guarded operator-authorized real-apply run scope.
# It does NOT let AI perform apply/core/route/shell/network/secret/git actions.
#
# D151 opens D152 Post Real Apply Verification Scope only.
...
print("D151 SANDBOX CANDIDATE GUARDED REAL APPLY RUN SCOPE BOOT: repo =", ROOT)

Path("runtime_experimental/sandbox_candidate_guarded_real_apply_run_scope.py").write_text(MODULE, encoding="utf-8")
Path("tests/test_d151_sandbox_candidate_guarded_real_apply_run_scope.py").write_text(TESTS, encoding="utf-8")

print("\n== compile ==")
sh([sys.executable, "-m", "py_compile", "runtime_experimental/sandbox_candidate_guarded_real_apply_run_scope.py"], check=True)

print("\n== unit tests ==")
sh([sys.executable, "-m", "unittest", "tests.test_d151_sandbox_candidate_guarded_real_apply_run_scope", "-v"], check=True)

print("\n== run D151 ==")
subprocess.run([
    sys.executable, "-c",
    "from runtime_experimental.sandbox_candidate_guarded_real_apply_run_scope import create_sandbox_candidate_guarded_real_apply_run_scope\n"
    "r=create_sandbox_candidate_guarded_real_apply_run_scope()\n"
...
print("\n== report preview ==")
p = Path("reports/d151_sandbox_candidate_guarded_real_apply_run_scope.json")
if p.exists():
...
try:
    ...
    if self_rel.name == "D151_SANDBOX_CANDIDATE_GUARDED_REAL_APPLY_RUN_SCOPE_BOOT.py":
...
if status.stdout.strip():
    commit = subprocess.run(["git", "commit", "-m", "bridge: add D151 sandbox candidate guarded real apply run scope"], text=True, capture_output=True)
    print(commit.stdout)
    if commit.stderr:
        print(commit.stderr)
else:
    print("No D151 changes to commit.")

print("\n== final status ==")
sh(["git", "status", "--short"])
print("\nD151 SANDBOX CANDIDATE GUARDED REAL APPLY RUN SCOPE BOOT DONE")
